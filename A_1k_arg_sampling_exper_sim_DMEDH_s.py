        ...
        for picNO in range(0, testNUM):
            with torch.no_grad():
                ...
                def add_noise_to_hologram(hologram, snr_db):
                    ...
                    # 根据信噪比计算噪声方差 # 信噪比公式：SNR(dB) = 10 * log10(signal_power / noise_power) # 重写为：noise_power = signal_power / (10 ** (snr_db / 10))
                    noise_power = signal_avg / (10 ** (snr_db / 10))#db20=0.01noise_power;db40=0.0001noise_power
                    print(noise_power)
                    noise_std = np.sqrt(noise_power)#noise_std 0.01;noise_power 0.0001
                    # 生成并添加高斯噪声
                ...
                # amp_gt = ampBatch[img_index, :, :]#0 8 9
                ...
                for kk in range(startStep, endStep):
                    ...
                    if planes == 5:
                        ...
                        # 构建support约束进行局部取样（只取中间得到的部分）转为复数图
                        ...
                        if useNet:
                            for iii in range(0, 40):
                                inIn = getInsideIndex(1400, 512)  # 用于NET
                                amp_phase = np.concatenate(
                                    [object0[None, inIn:inIn + 512, inIn:inIn + 512],
                                     ph0[None, inIn:inIn + 512, inIn:inIn + 512]],
                                    axis=0)
                                amp_phase = np.float32(amp_phase)
                                amp_phase = torch.from_numpy(amp_phase[None, :, :, :]).cuda()
                                amp_phase1, amp_phase = predictor_update_fn(amp_phase, vec_t, model=img_model)
                                # amp_phase1, amp_phase = corrector_update_fn(amp_phase, vec_t, model=img_model)#test
                                amp_phase = toNumpy(amp_phase)
                                object0[inIn:inIn + 512, inIn:inIn + 512] = amp_phase[0]
                                ph0[inIn:inIn + 512, inIn:inIn + 512] = amp_phase[1]

                        t0 = object0 * np.exp(1j * ph0)

                        # 平面5的复数图使用平面4的prop111实现更新至平面4的全息图，并获取平面4的初始幅值和相位信息
                        field_final_updated0 = IFT((FT(t0)) * (prop000))
                        amplitude0 = abs(field_final_updated0)
                        phase0 = np.angle(field_final_updated0)
                    ...
                    print(f"kk {kk}次")
                    recon_croped = crops(recon, 64, N2)
                    savePng(
                        f'./{savePath}/pic/{title}_planes{planes}/{picNO}/abs(recon_croped)/abs(recon_croped)_{kk}.png',
                        abs(recon_croped))
                    savePng(
                        f'./{savePath}/pic/{title}_planes{planes}/{picNO}/np.angle(recon_croped)/np.angle(recon_croped)_{kk}.png',
                        np.angle(recon_croped))

    ...
    def pc_sampler(img_model, check_num, predict, correct):
        ...
        for picNO in range(0, testNUM):
            with torch.no_grad():
                ...
                savePath = 'optics_p_3'  #########保存图片位置，需要自己设置
                ...
                hologram_bkg[hologram_bkg == 0] = np.finfo(float).eps
                ...
                for kk in range(startStep, endStep):
                    ...
                    t = timesteps[1200]#800 #1200 625 #1000 400  404-1200 403-800 #800 for size 1000 600 for size 1000
                    step = kk
                    ...
                    if planes == 5:
                        ...
                        # 全息图反向转播变成稀疏的复数图并使用了prop444进行外推
                        field_final4 = amplitude4 * np.exp(1j * phase4)
                        ...
                        savePng(
                            f'./{savePath}/pic/{title}_planes{planes}/{kk}/tw_abs.png',
                            np.abs(t_w))
                        savePng(
                            f'./{savePath}/pic/{title}_planes{planes}/{kk}/tw_angle.png',
                            np.angle(t_w))
                        ...
                        savePng(
                            ...
                            np.abs(t_w))
                        savePng(
                            f'./{savePath}/pic/{title}_size_planes{planes}/{kk}/tw_angle.png',
                            np.angle(t_w))
                        field_final_updated4 = IFT((FT(t_w)) * (prop444))
                        amplitude4 = abs(field_final_updated4)
                        phase4 = np.angle(field_final_updated4)
